...
All three base models now use class-weighted training to improve detection of
rare attack types (R2L, U2R) which are severely under-represented even after SMOTE.
...
def main():
    ...
    # ---- Compute class weights (shared across all models) ----
    classes = np.unique(y_base)
    weights = compute_class_weight("balanced", classes=classes, y=y_base)
    class_weight_dict = dict(zip(classes, weights))
    print(f"Class weights: {class_weight_dict}")

    # Compute per-sample weights for XGBoost (which doesn't accept class_weight)
    sample_weights = np.array([class_weight_dict[label] for label in y_base])
    ...
    xgb.fit(X_base, y_base, sample_weight=sample_weights)
